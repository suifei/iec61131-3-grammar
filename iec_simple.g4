...
variable_spec
    : identifier_list COLON data_type (ASSIGN array_initialization |ASSIGN expression)? SEMICOLON
    ;
...
array_type
    ...
    | ARRAY L_BRACKET INT_LITERAL DOT DOT INT_LITERAL R_BRACKET OF data_type  // Array with range [1..12]
    | ARRAY L_BRACKET IDENTIFIER DOT DOT IDENTIFIER R_BRACKET OF data_type  // Array with identifiers [min..max]
    | ARRAY L_BRACKET DOT_DOT R_BRACKET OF data_type  // Dynamic array [..]
    ;

array_initialization
    : L_BRACKET array_initial_elements (COMMA array_initial_elements)* R_BRACKET
    ;

array_initial_elements
    : array_initial_element 
    | INT_LITERAL L_PAREN (array_initial_element)? R_PAREN  // Repeated elements
    ;

array_initial_element
    : constant
    | identifier  // enumerated_value
    | struct_constructor
    | array_initialization
    ;

subrange
    : integer_literal DOT_DOT integer_literal
    | identifier DOT_DOT identifier
    ;
...
type_spec
    : identifier COLON enum_spec SEMICOLON
    | identifier COLON data_type SEMICOLON
    ;
...
multi_element_access
    ...
    | L_BRACKET expression DOT_DOT expression R_BRACKET
    ;
...
expr_mul
    : expr_unary ((MULTIPLY | DIVIDE | MOD) expr_unary)*
    ;

expr_unary
    : (NOT | MINUS) expr_unary
    | expr_primary
    ;
...
func_call
    ...
    | identifier L_PAREN param_expression (COMMA param_expression)* R_PAREN
    | std_func_call
    ;

std_func_call
    : (MIN | MAX | TRUNC) L_PAREN expression (COMMA expression)* R_PAREN
    ;
...
comparison_operator
    : EQUALS | NOT_EQUALS | LESS_THAN | GREATER_THAN | LESS_THAN_EQUAL | GREATER_THAN_EQUAL
    ;
...
REAL_LITERAL : [0-9]+ DOT [0-9]* | DOT [0-9]+;
STRING_LITERAL : '\'' (~'\'')* '\'';
TIME_LITERAL : 't#' (~'#')* | 'T#' (~'#')*;
